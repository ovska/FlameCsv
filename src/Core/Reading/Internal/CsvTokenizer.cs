        ...
        where T : unmanaged, IBinaryInteger<T>
    {
        ...
        if (Vec256.IsSupported)
        {
            return options.Newline.IsCRLF()
                ? new SimdTokenizer<T, NewlineCRLF, Vec256>(options)
                : new SimdTokenizer<T, NewlineLF, Vec256>(options);
        }

        if (Vec512.IsSupported)
        {
            return options.Newline.IsCRLF()
                ? new SimdTokenizer<T, NewlineCRLF, Vec512>(options)
                : new SimdTokenizer<T, NewlineLF, Vec512>(options);
        }

        if (Vec128.IsSupported)
        {
            return options.Newline.IsCRLF()
                ? new SimdTokenizer<T, NewlineCRLF, Vec128>(options)
                : new SimdTokenizer<T, NewlineLF, Vec128>(options);
        }
