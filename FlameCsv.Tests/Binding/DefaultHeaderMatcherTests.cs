...
public static class DefaultHeaderMatcherTests
{
    ...
    {
        var fn = DefaultHeaderMatchers.MatchText(StringComparison.Ordinal);
    ...
    {
        var fn = DefaultHeaderMatchers.MatchText(StringComparison.OrdinalIgnoreCase);
        var member = typeof(Shim).GetProperty("Prop")!;
        Assert.Equal(new CsvBinding(0, member), fn(GetArgs(0, member), "Prop"));
    ...
    {
        var fn = DefaultHeaderMatchers.MatchUtf8(StringComparison.Ordinal);
        var member = typeof(Shim).GetProperty("Prop")!;
    ...
    {
        var fn = DefaultHeaderMatchers.MatchUtf8(StringComparison.OrdinalIgnoreCase);
        var member = typeof(Shim).GetProperty("Prop")!;
        Assert.Equal(new CsvBinding(0, member), fn(GetArgs(0, member), U8("Prop")));
    ...
    {
        Assert.ThrowsAny<ArgumentException>(() => DefaultHeaderMatchers.MatchText((StringComparison)int.MaxValue));
        Assert.ThrowsAny<ArgumentException>(() => DefaultHeaderMatchers.MatchUtf8((StringComparison)int.MaxValue));
    }
